...
Deno.serve(async (req) => {
  try {
    ...
    subcontractors.forEach((sub, idx) => {
      ...
      doc.setFont('Helvetica', 'normal');
      doc.setFontSize(10);
      doc.text(sub.w9_full_name || sub.name || '___________________________________', margin + 2, yPos);
      yPos += 8;
      ...
      doc.setFont('Helvetica', 'normal');
      doc.setFontSize(10);
      doc.text(sub.w9_business_name || sub.company || '___________________________________', margin + 2, yPos);
      yPos += 8;
      ...
      const boxes = [
        { text: 'Individual/sole proprietor or single-member LLC', type: 'individual', x: margin + 2 },
        { text: 'C Corporation', type: 'c_corp', x: margin + 80 },
        { text: 'S Corporation', type: 's_corp', x: margin + 2, offset: 5 },
        { text: 'Partnership', type: 'partnership', x: margin + 80, offset: 5 },
        { text: 'Trust/estate', type: 'trust', x: margin + 2, offset: 10 },
        { text: 'Limited liability company. Enter the tax classification:', type: 'llc', x: margin + 80, offset: 10 }
      ];
      ...
      if (entityType === 'individual' || entityType === 'sole_proprietor') {
        doc.text('☒ Individual/sole proprietor or single-member LLC', margin + 2, yPos);
      } else if (entityType === 'c_corp') {
        doc.text('☒ C Corporation', margin + 80, yPos);
      } else if (entityType === 's_corp') {
        doc.text('☒ S Corporation', margin + 2, yPos + 5);
      } else if (entityType === 'partnership') {
        doc.text('☒ Partnership', margin + 80, yPos + 5);
      } else if (entityType === 'llc') {
        doc.text('☒ Limited liability company. Enter tax classification: _____', margin + 80, yPos + 10);
      }
      ...
      doc.setFont('Helvetica', 'normal');
      doc.setFontSize(10);
      doc.text(sub.address || '___________________________________', margin + 2, yPos);
      yPos += 8;
