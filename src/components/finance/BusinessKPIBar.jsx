    ...
    const items = jobs
      ...
      .map(j => {
        ...
        return outstanding > 0 ? {
          label: j.title || "Job",
          sublabel: `Client: ${j.client_name || "—"} · Contract: ${formatCurrency(j.contract_amount || 0)} · Collected: −${formatCurrency(collected)}${writeOff > 0 ? ` · Written off: −${formatCurrency(writeOff)}` : ""}`,
          amount: outstanding,
  ...
  const buildProjectedGrossProfitItems = () => {
    const jobItems = jobs
      ...
      .map(j => {
        ...
        return income > 0 ? {
          label: j.title || "Job",
          sublabel: `Client: ${j.client_name || "—"} · Status: ${j.status} · Contract: ${formatCurrency(j.contract_amount || 0)}${jobCOs > 0 ? ` + COs: ${formatCurrency(jobCOs)}` : ""}${writeOff > 0 ? ` − Write-off: ${formatCurrency(writeOff)}` : ""}`,
          amount: income,
          amountColor: "text-green-600",
        } : null;
      }).filter(Boolean);
  ...
  return (
    <>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <KPI label="Projected Total Income" value={formatCurrency(projectedGrossProfit)} sub="Sum of active job contracts + COs" icon={TrendingUp} color={projectedGrossProfit >= 0 ? "text-green-500" : "text-red-500"} onClick={() => setModal(buildProjectedGrossProfitItems())} />
